...
class TestRoles(unittest.TestCase):
    def test_grant_and_revoke_role(self):
        ...
        sdk = ThirdwebSdk(
            SdkOptions(private_key=environ["PKEY"]), "https://rpc-mumbai.maticvigil.com"
        )

        modules = [
            sdk.get_nft_module(TEST_NFT_CONTRACT_ADDRESS),
            sdk.get_market_module(TEST_MARKET_CONTRACT_ADDRESS),
            sdk.get_bundle_module(TEST_BUNDLE_CONTRACT_ADDRESS),
            sdk.get_pack_module(TEST_PACK_CONTRACT_ADDRESS),
            sdk.get_currency_module(TEST_CURRENCY_CONTRACT_ADDRESS),
        ]
