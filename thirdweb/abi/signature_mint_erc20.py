"""Generated wrapper for SignatureMintERC20 Solidity contract."""
...
# constructor for SignatureMintERC20 below.
try:
    # both mypy and pylint complain about what we're doing here, but this
    # works just fine, so their messages have been disabled here.
    from . import (  # type: ignore # pylint: disable=import-self
        SignatureMintERC20Validator,
    )
except ImportError:

    class SignatureMintERC20Validator(  # type: ignore
        Validator
    ):
...
class ISignatureMintERC20MintRequest(TypedDict):
...
class MintWithSignatureMethod(ContractMethod): # pylint: disable=invalid-name
    ...
    def validate_and_normalize_inputs(self, req: ISignatureMintERC20MintRequest, signature: Union[bytes, str]):
    ...
    def call(self, req: ISignatureMintERC20MintRequest, signature: Union[bytes, str], tx_params: Optional[TxParams] = None) -> None:
    ...
    def send_transaction(self, req: ISignatureMintERC20MintRequest, signature: Union[bytes, str], tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
    ...
    def build_transaction(self, req: ISignatureMintERC20MintRequest, signature: Union[bytes, str], tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (req, signature) = self.validate_and_normalize_inputs(req, signature)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(req, signature).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, req: ISignatureMintERC20MintRequest, signature: Union[bytes, str], tx_params: Optional[TxParams] = None) -> int:
...
class VerifyMethod(ContractMethod): # pylint: disable=invalid-name
    ...
    def call(self, req: ISignatureMintERC20MintRequest, signature: Union[bytes, str], tx_params: Optional[TxParams] = None) -> Tuple[bool, str]:
        """Execute underlying contract method via eth_call.
...
class SignatureMintERC20:
    """Wrapper class for SignatureMintERC20 Solidity contract.
    ...
    def __init__(
        ...
        validator: SignatureMintERC20Validator = None,
    ):
        ...
        if not validator:
            validator = SignatureMintERC20Validator(web3_or_provider, contract_address)
        ...
        functions = self._web3_eth.contract(address=to_checksum_address(contract_address), abi=SignatureMintERC20.abi()).functions
    ...
    def get_tokens_minted_with_signature_event(
        ...
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=SignatureMintERC20.abi()).events.TokensMintedWithSignature().processReceipt(tx_receipt)
