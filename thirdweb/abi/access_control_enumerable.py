"""Generated wrapper for AccessControlEnumerable Solidity contract."""
...
# constructor for AccessControlEnumerable below.
try:
    # both mypy and pylint complain about what we're doing here, but this
    # works just fine, so their messages have been disabled here.
    from . import (  # type: ignore # pylint: disable=import-self
        AccessControlEnumerableValidator,
    )
except ImportError:

    class AccessControlEnumerableValidator(  # type: ignore
        Validator
    ):
...
class AccessControlEnumerable:
    """Wrapper class for AccessControlEnumerable Solidity contract."""
    ...
    def __init__(
        ...
        validator: AccessControlEnumerableValidator = None,
    ):
        ...
        if not validator:
            validator = AccessControlEnumerableValidator(web3_or_provider, contract_address)
        ...
        functions = self._web3_eth.contract(address=to_checksum_address(contract_address), abi=AccessControlEnumerable.abi()).functions
    ...
    def get_role_admin_changed_event(
        ...
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=AccessControlEnumerable.abi()).events.RoleAdminChanged().processReceipt(tx_receipt)
    def get_role_granted_event(
        ...
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=AccessControlEnumerable.abi()).events.RoleGranted().processReceipt(tx_receipt)
    def get_role_revoked_event(
        ...
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=AccessControlEnumerable.abi()).events.RoleRevoked().processReceipt(tx_receipt)
