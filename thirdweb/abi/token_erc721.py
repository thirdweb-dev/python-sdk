...
class TokenERC721:
    ...
    def get_funds_withdrawn_event(
        self, tx_hash: Union[HexBytes, bytes]
    ) -> Tuple[AttributeDict]:
        """Get log entry for FundsWithdrawn event.

        :param tx_hash: hash of transaction emitting FundsWithdrawn event
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=TokenERC721.abi()).events.FundsWithdrawn().processReceipt(tx_receipt)
