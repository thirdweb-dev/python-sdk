...
try:
    ...
    class ContractMetadataRegistryValidator(  # type: ignore
        Validator
    ):
...
class RegisterMetadataMethod(ContractMethod): # pylint: disable=invalid-name
    ...
    def validate_and_normalize_inputs(self, contract_address: str, metadata_uri: str):
    ...
    def call(self, contract_address: str, metadata_uri: str, tx_params: Optional[TxParams] = None) -> None:
        """Execute underlying contract method via eth_call.

        :param tx_params: transaction parameters
        :returns: the return value of the underlying method.
        """
        (contract_address, metadata_uri) = self.validate_and_normalize_inputs(contract_address, metadata_uri)
        tx_params = super().normalize_tx_params(tx_params)
        self._underlying_method(contract_address, metadata_uri).call(tx_params.as_dict())

    def send_transaction(self, contract_address: str, metadata_uri: str, tx_params: Optional[TxParams] = None) -> Union[HexBytes, bytes]:
        """Execute underlying contract method via eth_sendTransaction.
        ...
        (contract_address, metadata_uri) = self.validate_and_normalize_inputs(contract_address, metadata_uri)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(contract_address, metadata_uri).transact(tx_params.as_dict())

    def build_transaction(self, contract_address: str, metadata_uri: str, tx_params: Optional[TxParams] = None) -> dict:
        """Construct calldata to be used as input to the method."""
        (contract_address, metadata_uri) = self.validate_and_normalize_inputs(contract_address, metadata_uri)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(contract_address, metadata_uri).buildTransaction(tx_params.as_dict())

    def estimate_gas(self, contract_address: str, metadata_uri: str, tx_params: Optional[TxParams] = None) -> int:
        """Estimate gas consumption of method call."""
        (contract_address, metadata_uri) = self.validate_and_normalize_inputs(contract_address, metadata_uri)
        tx_params = super().normalize_tx_params(tx_params)
        return self._underlying_method(contract_address, metadata_uri).estimateGas(tx_params.as_dict())
...
class ContractMetadataRegistry:
    ...
    def __init__(
        ...
        if not validator:
            validator = ContractMetadataRegistryValidator(web3_or_provider, contract_address)
        ...
        functions = self._web3_eth.contract(address=to_checksum_address(contract_address), abi=ContractMetadataRegistry.abi()).functions
        ...
        self.get_metadata_uri = GetMetadataUriMethod(web3_or_provider, contract_address, functions.getMetadataUri, validator)
        ...
        self.register_metadata = RegisterMetadataMethod(web3_or_provider, contract_address, functions.registerMetadata, validator)
    ...
    def get_metadata_registered_event(
        ...
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=ContractMetadataRegistry.abi()).events.MetadataRegistered().processReceipt(tx_receipt)
    def get_role_admin_changed_event(
        ...
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=ContractMetadataRegistry.abi()).events.RoleAdminChanged().processReceipt(tx_receipt)
    def get_role_granted_event(
        ...
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=ContractMetadataRegistry.abi()).events.RoleGranted().processReceipt(tx_receipt)
    def get_role_revoked_event(
        ...
        """
        tx_receipt = self._web3_eth.getTransactionReceipt(tx_hash)
        return self._web3_eth.contract(address=to_checksum_address(self.contract_address), abi=ContractMetadataRegistry.abi()).events.RoleRevoked().processReceipt(tx_receipt)
